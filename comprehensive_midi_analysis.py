...
class MidiAnalyzer:
    ...
    def analyze_note_spacing(self) -> Dict[str, Any]:
        ...
        results.update({
            'total_duration': max(n['end_time'] for n in sorted_notes) - min(n['start_time'] for n in sorted_notes),
        ...
        print(f"Average note duration: {results['avg_note_duration']:.2f}")
        ...
        if intervals:
            ...
            print("\nInterval Analysis:")
            print(f"Average interval: {results['avg_interval']:.3f}")
            print(f"Median interval: {results['median_interval']:.3f}")
            print(f"Min/Max intervals: {results['min_interval']:.3f}/{results['max_interval']:.3f}")
    ...
    def analyze_note_organization(self) -> Dict[str, Any]:
        ...
        results['channels_used'] = int(len(channels_used))
        channel_dist_dict = dict(channel_counts)
        results['channel_distribution'] = channel_dist_dict
        ...
        if all_chords:
            ...
            print(f"  Chord timing intervals: {len(results['chord_intervals'])}")
        else:
            print("\nNO CHORDS DETECTED")
    ...
    def analyze_rhythmic_patterns(self) -> Dict[str, Any]:
        ...
        if len(start_times) >= 10:  # Need enough notes for pattern analysis
            ...
            for interval in possible_intervals:
                ...
                for i in range(1, len(start_times)):
                    ...
                    # Check if timing is close to interval (within 5%)
                    if abs((diff / interval) % 1.0) < 0.05 or abs(((diff / interval) % 1.0) - 1.0) < 0.05:
                        matches += 1

                regularity_score = matches / total_checks if total_checks > 0 else 0
                regularity_scores[interval] = regularity_score
                print(f"Regularity for {interval}s interval: {regularity_score:.1f}")
        return results
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        if 'intervals_greater_than_2s' in spacing_analysis:
            ...
            if large_gaps_pct > 20:
                print(f"WARNING: HIGH SPACING ISSUE: {large_gaps_pct:.1f}% of intervals > 2s")

        # Chord issues
        if organization_analysis.get('total_simultaneous_notes', 0) == 0:
            print("WARNING: NO CHORDS DETECTED - All notes are monophonic!")

        # Rhythm issues
        if 'avg_interval' in spacing_analysis:
            avg_interval = spacing_analysis['avg_interval']
            if avg_interval > 1.0:
                print("WARNING: SPACING ISSUE: Average interval is long (>1s)")
            elif avg_interval > 0.5:
                print("OK: Note spacing is moderate but could be more rhythmic")
            else:
                print("GOOD: Notes are closely spaced - good rhythmic density")
...
def compare_midi_files(file_list: List[str]) -> Dict[str, Any]:
    """Compare multiple MIDI files for patterns."""
    reports = []
    ...
    for filename in file_list:
        if os.path.exists(filename):
            ...
            print('='*80)
            report = analyze_midi_file(filename)
    ...
    if len(reports) > 1:
        ...
        avg_intervals = [r['spacing_analysis'].get('avg_interval', 0) for r in reports if r['spacing_analysis'].get('avg_interval')]
        avg_densities = [r['spacing_analysis'].get('note_density_per_second', 0) for r in reports]
        chord_counts = [r['organization_analysis'].get('total_simultaneous_notes', 0) for r in reports]

        if avg_intervals:
            print(f"Average intervals: {statistics.mean(avg_intervals):.3f}")
        if avg_densities:
            print(f"Average note densities: {statistics.mean(avg_densities):.3f}")
        print(f"Average chord events: {statistics.mean(chord_counts):.1f}")

    return {'individual_reports': reports}

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python comprehensive_midi_analysis.py <midi_file> [<midi_file2> ...]")
        sys.exit(1)

    files_to_analyze = sys.argv[1:]

    if len(files_to_analyze) == 1:
        analyze_midi_file(files_to_analyze[0])
    else:
        compare_midi_files(files_to_analyze)