...
def generate_song(genre, mood="happy", tempo=120, bars=32, filename=None, separate_files=False):
    ...
    """
    if filename is None:
        # Auto-generate: {genre}_{mood}_{tempo}_{bars}_{timestamp}.mid in output folder
        filename = MidiOutput().generate_output_filename(genre, mood, tempo, bars, output_folder="output")
    else:
...
def main():
    ...
    for genre, tempo, bars in genres:
        for mood in moods:
            filename = None  # Let the program auto-generate from settings + date
            try:
                generate_song(genre, mood, tempo, bars, filename)
            except Exception as e:
                print(f"Error generating song for {genre}-{mood}: {e}")
