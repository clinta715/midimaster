...
try:
    ...
    IMPORT_ERROR = e  # Do not exit during test collection; tests will handle availability
    # Skip the entire module during pytest collection if dependencies are missing
    pytest.skip("Enhanced enumeration dependencies not available", allow_module_level=True)
...
def test_parameter_enumerator():
    ...
    try:
        ...
        for category, keywords in enumerator.category_keywords.items():
            print(f"      {category.value}: {keywords[:3]}...")
