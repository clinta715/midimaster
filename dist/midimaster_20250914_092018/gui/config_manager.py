...
class ConfigManager:
    ...
    def save_configuration(self, params: Dict[str, Any], name: str = "", description: str = "") -> str:
        ...
        if not name:
            ...
            genre = _san(params.get('genre', 'unknown'))
            mood = _san(params.get('mood', 'unknown'))
            tempo = int(params.get('tempo', 0)) if 'tempo' in params else 0
            bars = int(params.get('bars', 0)) if 'bars' in params else 0
            name = f"config_{genre}_{mood}_{tempo}_{bars}_{timestamp}"
