    ...
    def run(self):
        ...
        try:
            ...
            if self.preview_mode:
                ...
                output_path = str(self.params.get('output', '') or '').strip()
                if not output_path:
                    # Auto-generate: {genre}_{mood}_{tempo}_{bars}_{timestamp}.mid in output folder (default 'output')
                    output_path = midi_output.generate_output_filename(
                        genre=self.params.get('genre', ''),
                        mood=self.params.get('mood', ''),
                        tempo=self.params.get('tempo', 0),
                        bars=self.params.get('bars', 0),
                        output_folder=self.params.get('output_folder', None) or "output"
                    )
                    ...
                    if p.suffix.lower() != ".mid":
                        p = p.with_suffix(".mid")
                    # If relative simple name, join with provided output_folder (or default)
                    if not p.is_absolute() and not any(sep in str(p) for sep in ("/", "\\")):
                        folder = self.params.get('output_folder', None) or "output"
                        p = _P(folder) / p.name
                    output_path = str(p)
                    self.params['output'] = output_path
