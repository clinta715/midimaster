...
def select_rhythm_tracks(mid: mido.MidiFile, notes: List[Note]) -> Tuple[List[int], List[int]]:
    ...
    # Channel 9 (zero-based) is percussion
    for n in notes:
        if n.channel == 9:
    ...
    if not rhythm_track_indices:
        # Choose track with most notes in percussion pitch range
    ...
    # Channels present in rhythm tracks
    for n in notes:
        if n.track_index in rhythm_track_indices:
            rhythm_channels.add(n.channel)
...
def compute_alignment(rhythm_onsets: List[float], onset_bpms: List[float], genre: str) -> Tuple[int, float]:
    """
    Grid alignment: nearest beat grid of length b = 60/BPM at local onset time.
