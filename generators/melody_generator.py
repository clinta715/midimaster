...
class MelodyGenerator:
    ...
    def _choose_next_pitch(self, contour: str) -> int:
        """Choose the next pitch based on melodic contour and intervals.
        ...
        if not self.context.scale_pitches:
            # Fallback to C major scale if no scale established
        ...
        # Probabilities for intervals (favor smaller intervals)
        interval_probabilities = {1: 0.4, 2: 0.3, 3: 0.15, 4: 0.1, 5: 0.05}

        # Adjust probabilities based on contour
        if contour == 'rising':
        ...
        # Get possible next pitches
        possible_pitches = []
        for interval, prob in interval_probabilities.items():
        ...
        # Choose next pitch based on weighted probabilities
    ...
    def generate(self, num_bars: int) -> Pattern:
        ...
        # Choose a melodic contour for this pattern
        contour = random.choice(['rising', 'falling', 'arc', 'valley'])

        # Generate melody notes from the established scale
        # 4 beats per bar is the default time signature
        for i in range(num_bars * 4):
            pitch = self._choose_next_pitch(contour)
