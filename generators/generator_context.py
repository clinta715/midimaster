...
class GeneratorContext:
    ...
    def set_user_key_mode(self, key: str, mode: str) -> None:
        ...
        if not self.music_theory.validate_key_mode(key, mode):
            ...
            key_upper = key.upper()
            if key_upper not in [k.upper() for k in valid_keys]:
                raise ValueError(f"Invalid key: {key}. Valid keys: {', '.join(valid_keys)}. Try 'C major'.")
            else:
                raise ValueError(f"Invalid combination: {key} {mode}. Valid modes for {key}: {', '.join(valid_modes)}. Try '{key} major'.")
        self.user_key = key
        self.user_mode = mode
